...
app.post('/patRegister', async (req, res) => {
  ...
  const { patientId, aadharNumber, email, guardianName, emergencyContact, name, gender, contact, password, image } = req.body;

  if (!patientId || !aadharNumber || !email || !guardianName || !emergencyContact || !name || !gender || !contact || !password) {
  ...
  try {
    ...
    const patUser = new PatUser({ patientId, aadharNumber, email, guardianName, emergencyContact, name, gender, contact, password, image });
...
app.post('/orgRegister', async (req, res) => {
  ...
  try {
    ...
    if (orgExists) {
      return res.status(422).json({ error: "registration Number already exists" });
    }
...
app.post('/orgLogin', async (req, res) => {
  ...
  try {
    ...
    const organisationLogin = await OrgUser.findOne({ registrationNo: registrationNo });
